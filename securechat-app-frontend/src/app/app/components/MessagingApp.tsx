...
export default function MessagingApp({ user, onLogout }: MessagingAppProps) {
  ...
  const loadContacts = async () => {
    try {
      ...
      const contactsResponse = await fetch('https://52.53.221.141/contacts/', {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      
      // Load pending contacts
      const pendingResponse = await fetch('https://52.53.221.141/contacts/pending', {
        headers: { 'Authorization': `Bearer ${token}` }
      })
  ...
  const loadMessages = async () => {
    try {
      const token = localStorage.getItem('lockbox-token')
      const response = await fetch('https://52.53.221.141/messages/', {
        headers: {
  ...
  const loadChatRequests = async () => {
    ...
    try {
      ...
      const response = await fetch('https://52.53.221.141/chat-requests/incoming', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      // const recipientResponse = await fetch(`https://52.53.221.141/auth/keys/${activeContact.id}`, {
      //   headers: { 'Authorization': `Bearer ${token}` }
      // })
      ...
      const response = await fetch('https://52.53.221.141/messages/send', {
        method: 'POST',
        headers: {
  ...
  const loadContactMessages = async (contactId: string, silent: boolean = false) => {
    ...
    try {
      ...
      const response = await fetch(`https://52.53.221.141/messages/conversation/${contactId}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
