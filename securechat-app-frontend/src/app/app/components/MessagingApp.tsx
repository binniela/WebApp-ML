...
export default function MessagingApp({ user, onLogout }: MessagingAppProps) {
  ...
  const loadContacts = async () => {
    try {
      ...
      const contactsResponse = await fetch('http://10.201.154.21:8000/contacts/', {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      
      // Load pending contacts
      const pendingResponse = await fetch('http://10.201.154.21:8000/contacts/pending', {
        headers: { 'Authorization': `Bearer ${token}` }
      })
  ...
  const loadMessages = async () => {
    try {
      const token = localStorage.getItem('lockbox-token')
      const response = await fetch('http://10.201.154.21:8000/messages/', {
        headers: {
  ...
  const loadChatRequests = async () => {
    ...
    try {
      ...
      const response = await fetch('http://10.201.154.21:8000/chat-requests/incoming', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      // const recipientResponse = await fetch(`http://10.201.154.21:8000/auth/keys/${activeContact.id}`, {
      //   headers: { 'Authorization': `Bearer ${token}` }
      // })
      ...
      const response = await fetch('http://10.201.154.21:8000/messages/send', {
        method: 'POST',
        headers: {
  ...
  const loadContactMessages = async (contactId: string, silent: boolean = false) => {
    ...
    try {
      ...
      const response = await fetch(`http://10.201.154.21:8000/messages/conversation/${contactId}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
