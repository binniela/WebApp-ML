...
# Update NGINX configuration with proper message routing
...
server {
    ...
    # Message service routes - Map /messages/send to /send
    ...
    # Message service routes - Map /messages/ to /
    location /messages/ {
...
# Test the endpoints
echo "Testing /messages/send:"
curl -s -X POST http://localhost/messages/send -H "Content-Type: application/json" -H "Authorization: Bearer test" -d '{"test":"data"}'

echo -e "\nTesting /messages/:"
curl -s http://localhost/messages/ -H "Authorization: Bearer test"
EOF